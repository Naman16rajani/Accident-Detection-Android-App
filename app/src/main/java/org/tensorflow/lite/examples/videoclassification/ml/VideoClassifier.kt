...
class VideoClassifier private constructor(
    ...
    companion object {
        ...
        fun createFromFileAndLabelsAndOptions(
            ...
            val modelPath: MappedByteBuffer = FileUtil.loadMappedFile(context, modelFile)
            val interpreterOptions = Interpreter.Options().apply {
                setNumThreads(options.numThreads)
            }

            val interpreter = try {
                Interpreter(modelPath, interpreterOptions)
            } catch (e: Exception) {
                Log.e("TAG", "Failed to load model: ${e.message}")
                throw IllegalArgumentException("Model file is invalid or not found.")
            }
            // Load labels
